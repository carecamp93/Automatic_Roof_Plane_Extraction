...
def main(dataset, ckpt_path, image_size, viz_base, save_base, infer_times):
    ...
    ckpt_args = ckpt['args']
    if dataset == 'outdoor':
        data_path = './data/outdoorEnschede3D/cities_dataset'
        det_path = './data/outdoorEnschede3D/det_final'
    ...
    for data_i, data in enumerate(test_dataloader):
        ...
        # visualize G.T.
        gt_path = os.path.join(viz_base, '{}_gt.png'.format(base_name))
        ...
        recon_path = os.path.join(viz_base, '{}_pred_corner.png'.format(base_name))
        ...
        if dataset == 's3d_floorplan':
            ...
            save_path = os.path.join(save_base, '{}.npy'.format(base_name))
        ...
        if image_size != 256:
            ...
            pred_corners_viz = pred_corners
        recon_path = os.path.join(viz_base, '{}_pred_edge.png'.format(base_name))
        ...
        print('Finish inference for sample No.{}'.format(base_name))
